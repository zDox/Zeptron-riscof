...
class Zeptron(pluginTemplate):
    __model__ = "Zeptron"

    # TODO: please update the below to indicate family,
    #       version, etc of your DUT.
    __version__ = "XXX"
    ...
    def __init__(self, *args, **kwargs):
        ...
        # run the tests in parallel on the DUT executable. Can also be used
        ...
        # running them on the target, then following variable should
        # be set to False
    ...
    def runTests(self, testList):
        ...
        # create an instance the makeUtil class that we will use to
        # create targets.
        make = utils.makeUtil(
            makefilePath=os.path.join(self.work_dir,
                                      "Makefile." + self.name[:-1])
        )

        # set the make command that will be used. The num_jobs parameter
        # was set in the __init__
        # function earlier
        make.makeCommand = "make -k -j" + self.num_jobs

        # we will iterate over each entry in the testList. Each entry node
        # will be refered to by the variable testname.
        for testname in testList:
            ...
            # the tests, then the "else" clause is executed below assigning
            # the sim command to simple no action echo statement.
            if self.target_run:
            ...
            # automatically for each new target that is added
            make.add_target(execute)

            # if you would like to exit the framework once the makefile
            # generation is complete uncomment the following line. Note this
            # will prevent any signature checking or report generation.
            # raise SystemExit
